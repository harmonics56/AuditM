...
import com.mfpe.service.AuthorizationServiceImpl;
...
public class AuditSeverityController {
	...
	public ResponseEntity<?> auditSeverity(@RequestHeader("Authorization") String jwt,
		...
		if(jwt.length()>0 && authorizationService.validateJwt(jwt)) {
			...
			auditRequest.setManagerName(authorizationServiceImpl.managerName);
			auditRequest.setProjectName(authorizationServiceImpl.projectName);
//			logger.info(authorizationServiceImpl.managerName);
//			logger.info(authorizationServiceImpl.projectName);
...
//			List<AuditQuestion> questionResponses = auditRequest.getAuditDetail().getAuditQuestions();
			...
			List<AuditQuestion> questionResponses = auditCheckListFeign.getAllauditCheckListQuestions(jwt);
